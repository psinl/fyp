...
export default class OfferReceived extends React.Component {

  static navigationOptions = ({navigation}) => {
    return {
      title: 'Offers from Others'
    };
  };
  ...
  onCollectionUpdate = ()=>{
    ...
    this.ref.where("receiver","==", firebase.auth().currentUser.email).get()
    .then((querySnapshot) => {
      const offers = [];
      querySnapshot.forEach((doc) => {
        const{itemId,imageUrl,sender,status} = doc.data();
        offers.push({
          key:doc.id,
          doc,
          itemId,
          ...
          renderItem={({item}) =>
            ...
            >
              ...
              <Image source={{uri:`${item.imageUrl}`}}
                ...
                <Text numberOfLines={1} style={styles.itemTitle}>From: { item.sender }</Text>
